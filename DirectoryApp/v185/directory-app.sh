...
const AppInner = () => {
  ...
  const [editingPendingCompany, setEditingPendingCompany] = useState(false);
    ...
    <div style={S.app}>
          ...
          <>
            ...
            <div style={S.formGrid}>
              ...
              <CompanyPicker
                ...
                selectedCompanyId={personForm.pCompanyId}
                onSelect={id => personForm.setPCompanyId(id)}
                onClear={() => { personForm.setPCompanyId(''); if (personForm.pendingCompany) personForm.setPendingCompany(null); }}
                allowCreate={!personForm.editId}
                onQuickAdd={text => { setQuickAddCompanyInitName(text); setQuickAddCompanyOpen(true); }}
                pendingCompany={personForm.pendingCompany}
                onEditPending={() => { setEditingPendingCompany(true); setQuickAddCompanyOpen(true); }}
              />
              <div><label style={S.label}>Title</label><input style={S.input} value={personForm.pTitle} onChange={e => personForm.setPTitle(e.target.value)} placeholder="e.g. Software Engineer" /></div>
            </div>
            <div style={S.formGrid}>
      ...
      {quickAddCompanyOpen && (
        <QuickAddCompanyDialog
          initialName={editingPendingCompany ? undefined : quickAddCompanyInitName}
          editCompany={editingPendingCompany ? personForm.pendingCompany : null}
          onCancel={() => { setQuickAddCompanyOpen(false); setEditingPendingCompany(false); setQuickAddCompanyInitName(''); }}
          onCreated={(company) => {
            personForm.setPendingCompany(company);
            personForm.setPCompanyId(company.id);
            if (editingPendingCompany) {
              addToast(`✏️ ${company.name} updated`);
            } else {
              addToast(`✨ ${company.name} added as pending company`);
            }
            setQuickAddCompanyOpen(false);
            setEditingPendingCompany(false);
  ...
  const confirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (entry?.type === 'person') {
        ...
        // Clear companyId link from the person's company
        if (entry.companyId) {
      ...
      if (entry?.type === 'company') {
        // Clear companyId on all persons linked to this company
        for (const p of allPersons) {
          if (p.companyId === id) {
            await saveEntry({ ...p, companyId: '', title: '' });
          }
        }
      }
    ...
    try {
      for (const id of ids) {
        ...
        if (entry?.type === 'person') {
          ...
          // Clear companyId link from the person's company
          if (entry.companyId && !ids.includes(entry.companyId)) {
            const company = await loadEntry(entry.companyId);
            if (company && company.type === 'company' && company.contactPersonIds.includes(id)) {
              await saveEntry({ ...company, contactPersonIds: company.contactPersonIds.filter(c => c !== id) });
            }
        ...
        if (entry?.type === 'company') {
          // Clear companyId on all persons linked to this deleted company
          for (const p of allPersons) {
            if (p.companyId === id && !ids.includes(p.id)) {
              await saveEntry({ ...p, companyId: '', title: '' });
            }
          }
        }
      ...
      <div style={{ ...S.card, cursor: 'default', ...((entry.type === 'person' && entry.status === 'deceased') || (entry.type === 'company' && entry.companyStatus === 'closed') ? { borderLeft: '4px solid #9e9e9e' } : {}) }}>
        ...
        {entry.type === 'person' && (
          <>
            ...
            {(() => {
              const company = entry.companyId ? entries.find(e => e.id === entry.companyId) as Company | undefined : undefined;
              return (entry.companyId || entry.title) ? (
                <CollapsibleSection id="professional" title="Professional Information" collapsed={collapsedSections.has('professional')} onToggle={() => toggleSection('professional')}>
                  ...
                  {entry.companyId && !company && <div style={S.detailRow}><span style={S.detailLabel}>Company</span><span style={{ color: colors.textSec, fontStyle: 'italic' }}>Company not found</span></div>}
                  {entry.title && <div style={S.detailRow}><span style={S.detailLabel}>Title</span><span>{entry.title}</span></div>}
                </CollapsibleSection>
              ) : null;
            })()}
          </>
        ...
        {entry.type === 'company' && (() => {
          // Merge contactPersonIds with persons who have companyId pointing here
          const linkedPersonIds = allPersons.filter(p => p.companyId === entry.id).map(p => p.id);
...
## Outstanding Tasks
- Show professional info (company/title) on directory list view cards
