...
import type {
  ...
  View, SortField, SortDir, TypeFilter, Household, StatusFilter,
} from './types';
...
import { saveEntry, loadEntry, loadAll, removeEntry, exportCsv, exportFullCsv, parseCsvFile, saveHousehold, loadHousehold, loadAllHouseholds, removeHousehold, saveImage, loadAllImages, removeImage } from './storage';
...
import { CollapsibleSection, useCollapsedSections } from './components/CollapsibleSection';
...
import upcomingBirthdaysSource from './components/UpcomingBirthdays.tsx?raw';
...
export const App = () => {
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (statusFilter === 'active') {
      list = list.filter(e => {
        if (e.type === 'person') return (e.status || 'active') === 'active';
        return (e.companyStatus || 'active') === 'active';
      });
      ...
      list = list.filter(e => {
        if (e.type === 'person') return (e.status || 'active') !== 'archived';
    ...
    try {
      for (const id of ids) {
        ...
        if (entry?.type === 'person') {
          ...
          for (const p of allPersons) {
            if (!ids.includes(p.id) && p.childIds.includes(id)) {
              await saveEntry({ ...p, childIds: p.childIds.filter(c => c !== id) });
            }
          }
          for (const e of entries) {
            if (e.type === 'company' && !ids.includes(e.id) && e.contactPersonIds.includes(id)) {
              await saveEntry({ ...e, contactPersonIds: e.contactPersonIds.filter(c => c !== id) });
            }
          }
          if (entry.householdId) {
            ...
            if (household) {
              ...
              if (remainingIds.length <= 1) {
                for (const mid of remainingIds) {
                  const member = await loadEntry(mid);
                  if (member && member.type === 'person') {
                    await saveEntry({ ...member, householdId: '', addresses: member.addresses.map(a => a.label === 'Household' ? { ...a, label: 'Home' } : a) });
                  }
                }
                await removeHousehold(household.id);
              } else {
  ...
  const handleBulkStatusChange = async (ids: string[], status: 'active' | 'deceased' | 'closed') => {
    try {
      for (const id of ids) {
        const entry = await loadEntry(id);
        if (!entry) continue;
  ...
  const handleBulkAssignHousehold = async (personIds: string[], householdId: string) => {
    try {
      const household = await loadHousehold(householdId);
      if (!household) return;
      const householdAddr = { ...household.address, isPrimary: true, label: 'Household' };
      ...
      for (const pid of personIds) {
        ...
        // Remove from old household if different — load fresh from storage to avoid stale state
        ...
        // Update person's householdId and sync primary address
      ...
      // Save updated household with new members
  ...
  const handleCreateAndAssignHousehold = async (personIds: string[], household: Household) => {
    try {
      const householdAddr = { ...household.address, isPrimary: true, label: 'Household' };
...
`;
    ...
    const files: [string, string][] = [
      ...
      ['src/components/FamilyTree.tsx', familyTreeSource],
      ['src/components/CollapsibleSection.tsx', collapsibleSectionSource],
      ['src/components/UpcomingBirthdays.tsx', upcomingBirthdaysSource],
      ['src/components/VCardExport.tsx', vcardExportSource],
      ['src/components/BulkActions.tsx', bulkActionsSource],
      ['src/components/PrintDirectory.tsx', printDirectorySource],
      ['DEVLOG.md', devLog],
    ];
        ...
        {view === 'list' && (
          <>
            <div style={S.btnRow}>
              ...
              <button style={{ ...S.btn, ...(selectMode ? { background: colors.primary, color: '#fff' } : S.btnSec) }}
            ...
            <FilterBar sortField={sortField} setSortField={setSortField} sortDir={sortDir} setSortDir={setSortDir}
              ...
              statusFilter={statusFilter} setStatusFilter={setStatusFilter} totalCount={filteredEntries.length} />

            <UpcomingBirthdays persons={allPersons} images={images} onSelect={id => { setSelectedId(id); setView('detail'); }} />
            ...
            {loading ? <div style={S.emptyState}>Loading...</div> :
              filteredEntries.length === 0 ? <div style={S.emptyState}>{searchQ ? 'No matching entries.' : 'No entries yet. Add a person or company to get started.'}</div> :
                <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      {selectMode && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <button onClick={() => setSelectedIds(new Set(filteredEntries.map(e => e.id)))}
                            style={{ ...S.btn, padding: '2px 8px', fontSize: 11, ...S.btnSec }}>All</button>
                          <button onClick={() => setSelectedIds(new Set())}
                            style={{ ...S.btn, padding: '2px 8px', fontSize: 11, ...S.btnSec }}>None</button>
                        </div>
                      )}
                  ...
                  const inactiveBadge = e.type === 'person' ? '✝ DECEASED' : '🚫 CLOSED';
                  const isSelected = selectedIds.has(e.id);
                  return (
                  <div key={e.id} style={{ ...S.card, ...(isInactive ? { opacity: 0.7, borderLeft: '4px solid #9e9e9e' } : {}), ...(isSelected ? { border: `2px solid ${colors.primary}`, background: '#f0f7ff' } : {}) }}
                    ...
                    onClick={() => {
                      if (selectMode) {
                        setSelectedIds(prev => { const n = new Set(prev); if (n.has(e.id)) n.delete(e.id); else n.add(e.id); return n; });
                      } else {
                        setSelectedId(e.id); setView('detail');
                      }
                    }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        {isInactive && <span style={{ ...S.badge, background: '#e0e0e0', color: '#616161', fontSize: 10 }}>{inactiveBadge}</span>}
                  ...
                  <BulkActionsBar
                    ...
                    onBulkAssignHousehold={handleBulkAssignHousehold}
                    onCreateAndAssignHousehold={handleCreateAndAssignHousehold}
                    onError={setError}
                  />
                </>
            }
          ...
          <>
            ...
            <div style={S.formGrid}>
              ...
              <div>
                ...
                <select style={S.input} value={pStatus} onChange={e => setPStatus(e.target.value as 'active' | 'deceased')}>
              ...
              {pStatus === 'deceased' && (
                <div>
                  ...
                  <input style={S.input} type="date" value={pDeceasedDate} onChange={e => setPDeceasedDate(e.target.value)} />
                </div>
              )}
            </div>
        ...
        {view === 'companyForm' && (
          <>
            ...
            <div style={S.formGrid}>
              ...
              <div>
                <label style={S.label}>Status</label>
                <select style={S.input} value={cStatus} onChange={e => setCStatus(e.target.value as 'active' | 'closed')}>
                  <option value="active">Active</option>
              ...
              {cStatus !== 'active' && (
                <div>
                  <label style={S.label}>Date Closed</label>
                  <input style={S.input} type="date" value={cClosedDate} onChange={e => setCClosedDate(e.target.value)} />
                </div>
              )}
            </div>
          ...
          <>
            <div style={S.btnRow}>
              ...
              <button style={{ ...S.btn, ...S.btnSec }} onClick={async () => {
                try { await downloadVCard(selectedEntry, images[selectedEntry.id]); }
            ...
            <div style={{ ...S.card, cursor: 'default', ...((selectedEntry.type === 'person' && selectedEntry.status === 'deceased') || (selectedEntry.type === 'company' && selectedEntry.companyStatus === 'closed') ? { borderLeft: '4px solid #9e9e9e' } : {}) }}>
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14 }}>
                <div style={{ marginRight: 14 }}>
                  <ProfileImage imageUrl={images[selectedEntry.id] || null} size={64} fallback={selectedEntry.type === 'person' ? '👤' : '🏢'} />
                </div>
                <div>
                  <h2 style={{ margin: 0, ...((selectedEntry.type === 'person' && selectedEntry.status === 'deceased') || (selectedEntry.type === 'company' && selectedEntry.companyStatus === 'closed') ? { color: colors.textSec } : {}) }}>{getEntryName(selectedEntry)}</h2>
                  {selectedEntry.type === 'person' && selectedEntry.status === 'deceased' && (
                    <span style={{ ...S.badge, marginTop: 4, background: '#e0e0e0', color: '#616161' }}>
                      ✝ Deceased{selectedEntry.deceasedDate ? ` · ${selectedEntry.deceasedDate}` : ''}
                    </span>
                  )}
                  {selectedEntry.type === 'company' && selectedEntry.companyStatus === 'closed' && (
                    <span style={{ ...S.badge, marginTop: 4, background: '#e0e0e0', color: '#616161' }}>
                      🚫 Closed{selectedEntry.closedDate ? ` · ${selectedEntry.closedDate}` : ''}
                    </span>
                  )}
              ...
              {selectedEntry.type === 'person' && (
                <>
                  ...
                  <FamilyTree person={selectedEntry} allPersons={allPersons} images={images} onSelectPerson={id => setSelectedId(id)} />
                  </CollapsibleSection>
                  {(() => {
...
cat > "$ROOT/src/components/UpcomingBirthdays.tsx" << '__EOF_SRC_COMPONENTS_UPCOMINGBIRTHDAYS_TSX__'
import React from 'react';
import type { Person } from '../types';
import { colors } from '../styles';
import { ProfileImage } from './ProfileImage';
...
__EOF_SRC_COMPONENTS_UPCOMINGBIRTHDAYS_TSX__
...
export function BulkActionsBar({
  ...
  onClearSelection, onBulkDelete, onBulkStatusChange, onBulkAssignHousehold, onCreateAndAssignHousehold, onError,
}: BulkActionsBarProps) {
            ...
            <div style={S.dialogBody}>
              ...
              <div
                style={{
                  ...
                  border: `2px dashed ${colors.primary}`, cursor: 'pointer',
                  background: '#f0f7ff', textAlign: 'center',
                }}
                onMouseEnter={ev => (ev.currentTarget.style.background = '#e3effd')}
                onMouseLeave={ev => (ev.currentTarget.style.background = '#f0f7ff')}
                onClick={() => {
                ...
                <div style={{ fontWeight: 600, fontSize: 13, color: colors.primary }}>+ Create New Household</div>
...
│   ├── PrintDirectory.tsx           # Print dialog & HTML generation
│   ├── EmailInput.tsx               # Email input with blur validation
│   ├── PhoneInput.tsx               # Phone input with blur validation
│   ├── ProfileImage.tsx             # Profile image avatar with upload
│   ├── ImageCropper.tsx             # Interactive circular crop overlay
...
│   ├── UpcomingBirthdays.tsx        # Birthday widget for next 30 days
...
echo "✅ Done! Extracted 32 files into $ROOT/"
echo ""
echo "To get started:"
echo "  cd $ROOT"
echo "  npm install"
