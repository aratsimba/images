...
export const S: Record<string, React.CSSProperties> = {
  ...
  dropdownItem: { padding: '10px 14px', cursor: 'pointer', borderBottom: `1px solid ${colors.border}`, fontSize: 14, color: colors.text },
...
import { migrateEntry, toE164, getPrimary, selectNewPrimaryContact, capitalizeName } from './utils';
...
/**
 * Remove a person from their household, applying the correct rules:
 * - >2 members: remove person, reassign primary if needed, strip household addr
 * - ≤2 members: dissolve household, strip addr from removed person, convert
 *   other member's Household addr to Home
 *
 * @param personId - the person being removed
 * @param household - the household object
 * @param allPersons - all persons for primary contact selection
 * @returns 'dissolved' if the household was deleted, 'removed' otherwise
 */
export async function removePersonFromHousehold(
  personId: string,
  household: Household,
  allPersons: Person[],
): Promise<'dissolved' | 'removed'> {
  const remainingIds = household.memberIds.filter(mid => mid !== personId);

  // Helper: strip the Household address from a person, promote next address to primary
  const stripHouseholdAddr = (member: Person): Person => {
    const addrs = member.addresses.filter(a => a.label !== 'Household');
    // If we removed the primary, promote the first remaining address
    if (addrs.length > 0 && !addrs.some(a => a.isPrimary)) {
      addrs[0] = { ...addrs[0], isPrimary: true };
    }
    return { ...member, householdId: '', addresses: addrs };
  };

  if (remainingIds.length >= 2) {
    // >2 members: just remove person
    let newPrimary = household.primaryContactId;
    if (newPrimary === personId) {
      newPrimary = selectNewPrimaryContact(personId, remainingIds, allPersons);
    }
    // Strip household addr from the removed person
    const member = await loadEntry(personId);
    if (member && member.type === 'person') {
      await saveEntry(stripHouseholdAddr(member));
    }
    await saveHousehold({ ...household, memberIds: remainingIds, primaryContactId: newPrimary });
    return 'removed';
  } else {
    // ≤2 members: dissolve household
    // 2.2: Strip household addr from the removed person
    const removedMember = await loadEntry(personId);
    if (removedMember && removedMember.type === 'person') {
      await saveEntry(stripHouseholdAddr(removedMember));
    }
    // 2.3: For the other member(s), convert Household addr to Home
    for (const mid of remainingIds) {
      const other = await loadEntry(mid);
      if (other && other.type === 'person') {
        const addrs = other.addresses.map(a =>
          a.label === 'Household' ? { ...a, label: 'Home' } : a
        );
        await saveEntry({ ...other, householdId: '', addresses: addrs });
      }
    }
    // 2.4: Delete the household
...
export function csvRowToEntry(row: Record<string, string>): DirectoryEntry | null {
  ...
  if (type === 'person') {
    return migrateEntry({
      id, type: 'person',
      firstName: capitalizeName(row.firstName?.trim() || ''), lastName: capitalizeName(row.lastName?.trim() || ''),
...
import { EMAIL_LABELS_PERSON, EMAIL_LABELS_COMPANY, PHONE_LABELS_PERSON, PHONE_LABELS_COMPANY, GENDER_OPTIONS, INDUSTRY_OPTIONS } from './types';
import { S, colors } from './styles';
import { formatPhoneDisplay, getPrimary, getEntryName, formatAddr, getAncestorIds, getDescendantIds, capitalizeName, isValidIndustry } from './utils';
...
const AppInner = () => {
  ...
  const { entries, households, images, loading, error, setError, reload, allPersons, allAffiliatedCompanies } = data;
          ...
          {showSearchDropdown && searchQ && (
            <div style={S.dropdown}>
              ...
              {searchResults.slice(0, 10).map(e => {
                // For persons, find the matching affiliation context if the name/email didn't match
                let affContext = '';
                if (e.type === 'person' && searchQ) {
                  const sq = searchQ.toLowerCase();
                  const nameMatch = `${e.firstName} ${e.lastName} ${e.emails.map(x => x.address).join(' ')}`.toLowerCase().includes(sq);
                  if (!nameMatch && e.affiliations?.length) {
                    for (const aff of e.affiliations) {
                      const cName = allCompanies.find(c => c.id === aff.companyId)?.name || '';
                      if (aff.title && aff.title.toLowerCase().includes(sq)) { affContext = aff.title + (cName ? ` @ ${cName}` : ''); break; }
                      if (cName && cName.toLowerCase().includes(sq)) { affContext = (aff.title ? `${aff.title} @ ` : '') + cName; break; }
                    }
                  }
                }
                return (
                  <div key={e.id} style={{ ...S.dropdownItem, display: 'flex', alignItems: 'center', gap: 8 }} onMouseEnter={ev => (ev.currentTarget.style.background = colors.hover)} onMouseLeave={ev => (ev.currentTarget.style.background = '#fff')} onClick={() => { setSearchQ(getEntryName(e)); setShowSearchDropdown(false); setSelectedId(e.id); setView('detail'); }}>
                    ...
                    <span style={{ display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                      <span>{getEntryName(e)}{e.type === 'company' && e.industry && <span style={{ color: colors.textSec, marginLeft: 8, fontSize: 12 }}>({e.industry})</span>}</span>
                      {affContext && <span style={{ fontSize: 11, color: colors.textSec, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>🏢 {affContext}</span>}
                    </span>
                  </div>
                );
              })}
                    ...
                    <div key={e.id} style={{ ...S.card, ...(isInactive ? { opacity: 0.7, borderLeft: '4px solid #9e9e9e' } : {}), ...(isSelected ? { border: `2px solid ${colors.primary}`, background: '#f0f7ff' } : {}) }} onMouseEnter={ev => (ev.currentTarget.style.boxShadow = '0 3px 10px rgba(0,0,0,.12)')} onMouseLeave={ev => (ev.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,.08)')} onClick={() => { if (selectMode) { setSelectedIds(prev => { const n = new Set(prev); if (n.has(e.id)) n.delete(e.id); else n.add(e.id); return n; }); } else { setSelectedId(e.id); setView('detail'); } }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          {e.type === 'company' && e.industry && !isValidIndustry(e.industry) && <span style={{ ...S.badge, background: '#fff3e0', color: '#e65100', fontSize: 10 }} title={`"${e.industry}" is not a recognized industry`}>⚠️ UNRECOGNIZED</span>}
                          {e.type === 'company' && !e.industry && <span style={{ ...S.badge, background: '#fff3e0', color: '#e65100', fontSize: 10 }} title="No industry set">⚠️ NO INDUSTRY</span>}
        ...
        {view === 'companyForm' && (
          <>
            ...
            <div style={S.formGrid}>
              ...
              <div><label style={S.label}>Industry *</label><select ref={companyForm.cIndustryRef} style={{ ...S.input, ...(companyForm.fieldErrors.has('cIndustry') ? { borderColor: colors.danger } : {}) }} value={companyForm.cIndustry} onChange={e => { companyForm.setCIndustry(e.target.value); if (e.target.value !== 'Other') companyForm.setCIndustryOther(''); companyForm.setFieldErrors(prev => { const n = new Set(prev); n.delete('cIndustry'); n.delete('cIndustryOther'); return n; }); }}><option value="">— Select Industry —</option>{INDUSTRY_OPTIONS.map(ind => <option key={ind} value={ind}>{ind}</option>)}<option value="Other">Other</option></select>{companyForm.fieldErrors.has('cIndustry') && <div style={{ fontSize: 12, color: colors.danger, marginTop: 3 }}>Industry is required</div>}{companyForm.cIndustry === 'Other' && <><input style={{ ...S.input, marginTop: 8, ...(companyForm.fieldErrors.has('cIndustryOther') ? { borderColor: colors.danger } : {}) }} placeholder="Industry name" value={companyForm.cIndustryOther} onChange={e => { companyForm.setCIndustryOther(e.target.value); companyForm.setFieldErrors(prev => { const n = new Set(prev); n.delete('cIndustryOther'); return n; }); }} />{companyForm.fieldErrors.has('cIndustryOther') && <div style={{ fontSize: 12, color: colors.danger, marginTop: 3 }}>Please specify the industry</div>}</>}</div>
...
export function useFilteredEntries(
  ...
  // Build a map of company id → name for affiliation search
  const companyNameMap = useMemo(() => {
    const m = new Map<string, string>();
    for (const e of entries) if (e.type === 'company') m.set(e.id, e.name);
    return m;
  }, [entries]);

  const searchResults = useMemo(() => entries.filter(e => {
    if (e.type === 'person') {
      const base = `${e.firstName} ${e.lastName} ${e.emails.map(x => x.address).join(' ')}`;
      if (base.toLowerCase().includes(sq)) return true;
      // Search affiliation titles and affiliated company names
      if (e.affiliations?.length) {
        for (const aff of e.affiliations) {
          if (aff.title && aff.title.toLowerCase().includes(sq)) return true;
          const cName = companyNameMap.get(aff.companyId);
          if (cName && cName.toLowerCase().includes(sq)) return true;
        }
      }
      return false;
    }
    return `${e.name} ${e.industry} ${e.emails.map(x => x.address).join(' ')}`.toLowerCase().includes(sq);
  }), [entries, sq, companyNameMap]);
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (industryFilter) {
      if (industryFilter === 'Other') {
        list = list.filter(e => e.type === 'company' && (e.industry.trim() === 'Other' || e.industry.trim().startsWith('Other — ')));
      } else {
        list = list.filter(e => e.type === 'company' && e.industry.trim().toLowerCase() === industryFilter.toLowerCase());
      }
    }
...
export function usePersonForm(deps: PersonFormDeps) {
  ...
  const [pAffiliations, setPAffiliations] = useState<{ companyId: string; title: string }[]>([]);
  const [pIsMissionary, setPIsMissionary] = useState(false);
  ...
  const reset = () => {
    ...
    setPCompanyId(''); setPTitle(''); setPAffiliations([]); setPIsMissionary(false); setPendingCompanies([]);
  ...
  const fill = (p: Person) => {
    ...
    setPTitle(p.title || '');
    setPAffiliations(p.affiliations?.length ? [...p.affiliations] : (p.companyId ? [{ companyId: p.companyId, title: p.title || '' }] : []));
    setPIsMissionary(p.isMissionary || false);
    setPendingCompanies([]);
  ...
  const save = async () => {
    ...
    // Derive legacy companyId/title from first affiliation for backward compat
    // Filter out affiliations with no company selected
    const cleanAffiliations = pAffiliations.filter(a => a.companyId);
    const primaryAff = cleanAffiliations.length > 0 ? cleanAffiliations[0] : null;
    const person: Person = {
      ...
      companyId: primaryAff?.companyId || '', title: primaryAff?.title || '',
      affiliations: cleanAffiliations,
      isMissionary: pIsMissionary,
      notes: pNotes.trim(),
    ...
    try {
      ...
      const oldAffiliations = editId ? (allPersons.find(p => p.id === editId)?.affiliations || []) : [];
      const newAffCompanyIds = new Set(cleanAffiliations.map(a => a.companyId));
      const oldAffCompanyIds = new Set(oldAffiliations.map(a => a.companyId));
      // Remove person from companies no longer affiliated
      for (const oldAff of oldAffiliations) {
        if (!newAffCompanyIds.has(oldAff.companyId)) {
          const oldCompany = entries.find(e => e.id === oldAff.companyId);
      ...
      // Add person to newly affiliated companies
      for (const newAff of cleanAffiliations) {
        if (!oldAffCompanyIds.has(newAff.companyId)) {
          const existingCompany = entries.find(e => e.id === newAff.companyId);
      ...
      // Persist pending quick-added companies (with this person as contact)
      for (const pc of pendingCompanies) {
        if (newAffCompanyIds.has(pc.id)) {
          const companyToSave = !pc.contactPersonIds.includes(id)
            ? { ...pc, contactPersonIds: [...pc.contactPersonIds, id] }
            : pc;
  ...
  return {
    ...
    pAffiliations, setPAffiliations, pIsMissionary, setPIsMissionary,
    pendingCompanies, setPendingCompanies,
...
function SingleAddressFields({ addr, onChange, onRemove, canRemove, onSetPrimary, readOnly, hideSetPrimary, labels, lockLabel, allowCustomLabel }: {
  addr: Address; onChange: (a: Address) => void; onRemove: () => void; canRemove: boolean; onSetPrimary: () => void;
  readOnly?: boolean; hideSetPrimary?: boolean; labels: string[]; lockLabel?: boolean; allowCustomLabel?: boolean;
}) {
...
export function MultiAddressFields({ addresses, onChange, personMode, companyMode }: { addresses: Address[]; onChange: (a: Address[]) => void; personMode?: boolean; companyMode?: boolean }) {
...
export function RelationshipPicker({ label, entries, selectedIds, pendingIds, onToggle, onEditPending, onQuickAdd }: {
  label: string; entries: Person[]; selectedIds: string[]; pendingIds?: Set<string>; onToggle: (id: string) => void; onEditPending?: (id: string) => void; onQuickAdd?: (searchText: string) => void;
}) {
  ...
  return (
    <div style={S.fieldFull}>
      ...
      <input style={S.input} placeholder={`Search or create ${label.toLowerCase()}...`}
          ...
          {onQuickAdd && (
            <>
              ...
              <div style={{ ...S.dropdownItem, color: colors.primary, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}
                ...
                onMouseLeave={e => (e.currentTarget.style.background = '#fff')}
                onClick={() => { const text = q; setQ(''); onQuickAdd(text); }}>
...
interface FilterBarProps {
  ...
  missionaryFilter: boolean;
  setMissionaryFilter: (v: boolean) => void;
  companyFilter: string;
  setCompanyFilter: (v: string) => void;
  allAffiliatedCompanies: { id: string; name: string }[];
  totalCount: number;
}
...
export function FilterBar({
  ...
  missionaryFilter, setMissionaryFilter, companyFilter, setCompanyFilter,
  allAffiliatedCompanies, totalCount
}: FilterBarProps) {
  const [expanded, setExpanded] = useState(false);

  const hasActiveFilters = typeFilter !== 'all' || industryFilter !== '' || statusFilter !== 'active' || missionaryFilter || companyFilter !== '';
  const activeFilterCount = (typeFilter !== 'all' ? 1 : 0) + (industryFilter ? 1 : 0) + (statusFilter !== 'active' ? 1 : 0) + (missionaryFilter ? 1 : 0) + (companyFilter ? 1 : 0);

  const clearAll = () => {
    setTypeFilter('all');
    setIndustryFilter('');
    setStatusFilter('active');
    setMissionaryFilter(false);
    setCompanyFilter('');
  };
    ...
    <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            {typeFilter !== 'all' && (
              <span style={chipStyle}>
                ...
                <button style={chipRemoveStyle} onClick={() => { setTypeFilter('all'); setIndustryFilter(''); setMissionaryFilter(false); setCompanyFilter(''); }}>×</button>
              </span>
            )}
            {statusFilter !== 'active' && (
              <span style={chipStyle}>
            ...
            {missionaryFilter && (
              <span style={chipStyle}>
                ✝ Missionaries
                <button style={chipRemoveStyle} onClick={() => setMissionaryFilter(false)}>×</button>
              </span>
            )}
            {companyFilter && (
              <span style={chipStyle}>
                🏢 {allAffiliatedCompanies.find(c => c.id === companyFilter)?.name || 'Company'}
                <button style={chipRemoveStyle} onClick={() => setCompanyFilter('')}>×</button>
              </span>
            )}
      ...
      {expanded && (
        <div style={{
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {(['all', 'person', 'company'] as TypeFilter[]).map(t => (
                <button key={t} onClick={() => { setTypeFilter(t); if (t === 'company') { setIndustryFilter(''); setMissionaryFilter(false); setCompanyFilter(''); } else if (t === 'person') { setIndustryFilter(''); } else { /* 'all' — leave filters as-is */ } }}
                  style={{
          ...
          {/* Industry filter */}
          {(typeFilter === 'all' || typeFilter === 'company') && (
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: colors.textSec, marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                Industry
              </label>
          ...
          {(typeFilter === 'all' || typeFilter === 'person') && (
            <div>
              ...
              <button onClick={() => { setMissionaryFilter(!missionaryFilter); if (!missionaryFilter) { setTypeFilter('person'); setIndustryFilter(''); } }}
                style={{
                  width: '100%', padding: '6px 8px', fontSize: 12, fontWeight: 600,
                  border: `1px solid ${missionaryFilter ? '#7b1fa2' : colors.border}`,
                  borderRadius: 6, cursor: 'pointer',
                  background: missionaryFilter ? '#f3e5f5' : '#fff',
                  color: missionaryFilter ? '#7b1fa2' : colors.text,
                }}>
                {missionaryFilter ? '✝ Missionaries Only' : '✝ All'}
              </button>
            </div>
          )}

          {/* Company affiliation filter */}
          {allAffiliatedCompanies.length > 0 && (typeFilter === 'all' || typeFilter === 'person') && (
            <div>
              <label style={{ fontSize: 11, fontWeight: 700, color: colors.textSec, marginBottom: 6, display: 'block', textTransform: 'uppercase', letterSpacing: 0.5 }}>
                Affiliated With
              </label>
              <select value={companyFilter}
                onChange={e => { setCompanyFilter(e.target.value); if (e.target.value) { setTypeFilter('person'); setIndustryFilter(''); } }}
                style={{
                  width: '100%', padding: '6px 10px', borderRadius: 6,
                  border: `1px solid ${colors.border}`, fontSize: 12, outline: 'none',
                  background: '#fff', cursor: 'pointer',
                ...
                {allAffiliatedCompanies.map(c => <option key={c.id} value={c.id}>🏢 {c.name}</option>)}
              </select>
            </div>
          )}
        </div>
...
export function QuickAddRelativeDialog({ mode, currentGender, defaultLastName, initialFirstName, editPerson, onCreated, onCancel }: QuickAddRelativeProps) {
  ...
  const [firstName, setFirstName] = useState(() => {
    ...
    if (!isEdit && initialFirstName) return capitalizeName(initialFirstName);
    return '';
  });
  ...
  const handleSave = () => {
    ...
    const person: Person = {
      ...
      companyId: '', title: '', affiliations: [], isMissionary: false,
      notes: '',
...
cat > "$ROOT/src/components/CompanyPicker.tsx" << '__EOF_SRC_COMPONENTS_COMPANYPICKER_TSX__'
import React, { useState, useRef, useEffect } from 'react';
...
__EOF_SRC_COMPONENTS_COMPANYPICKER_TSX__

cat > "$ROOT/src/components/QuickAddCompany.tsx" << '__EOF_SRC_COMPONENTS_QUICKADDCOMPANY_TSX__'
import React, { useState, useRef, useEffect } from 'react';
import { v4 as uuidv4 } from 'uuid';
import { S, colors } from '../styles';
import { capitalizeName } from '../utils';
...
__EOF_SRC_COMPONENTS_QUICKADDCOMPANY_TSX__
...
echo "✅ Done! Extracted 43 files into $ROOT/"
