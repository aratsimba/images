          ...
          <>
            ...
            <div style={S.formGrid}>
              ...
              <HouseholdMembership allPersons={allPersons} households={households} editId={personForm.editId} selectedHouseholdId={personForm.pHouseholdId} onSelectHousehold={hid => { personForm.setPHouseholdId(hid); personForm.setHhCreateMode(false); personForm.setNewHhName(''); personForm.setHhNameError(''); }} onClearHousehold={() => { personForm.setPHouseholdId(''); personForm.setHhCreateMode(false); personForm.setNewHhName(''); personForm.setHhNameError(''); }}
                canCreateHousehold={!personForm.pHouseholdId && (!!personForm.pSpouseId || personForm.pChildIds.length > 0) && personForm.pAddrs.some(a => a.street.trim() && a.city.trim() && a.state.trim())}
                hasFamily={!personForm.pHouseholdId && (!!personForm.pSpouseId || personForm.pChildIds.length > 0)}
                familyMemberNames={(() => {
                ...
                onToggleCreateMode={on => { personForm.setHhCreateMode(on); if (!on) { personForm.setNewHhName(''); personForm.setHhNameError(''); } }}
...
export function usePersonForm(deps: PersonFormDeps) {
  ...
  const reset = () => {
    ...
    setPendingPersons([]); setHhCreateMode(false); setNewHhName(''); setHhNameError('');
  };
...
export function HouseholdView({ households, allPersons, images, onReload, onToast }: HouseholdViewProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      resetForm();
      onToast(wasEditing ? `✏️ ${savedName} updated` : `✅ ${savedName} created`);
    } catch (e) { if (e instanceof PageStorageError) setError((e as PageStorageError).message); }
  };
        ...
        <>
          ...
          <div style={S.formGrid}>
            <div style={S.fieldFull}>
              ...
              <input ref={nameRef} style={{ ...S.input, ...((fieldErrors.has('hhName') || fieldErrors.has('hhNameDupe')) ? { borderColor: colors.danger } : {}) }} value={name} onChange={e => { setName(e.target.value); setFieldErrors(prev => { const n = new Set(prev); n.delete('hhName'); n.delete('hhNameDupe'); return n; }); }} placeholder="e.g. The Smith Family" />
