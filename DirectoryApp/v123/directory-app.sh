...
export const App = () => {
  ...
  // ── Code / Log export ──
  const handleDownloadMarkdown = async () => { try { await downloadFile('directory-app-conversation.md', new Blob([devLog], { type: 'text/markdown' })); } catch (e: any) { setError(e?.message || 'Download failed'); } };
  const handleDownloadCode = async () => {
    ...
    try { await downloadFile('directory-app.sh', new Blob([lines.join('\n')], { type: 'text/x-shellscript' })); } catch (e: any) { setError(e?.message || 'Download failed'); }
  };
  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.body}>
        ...
        {view === 'personForm' && (
          <>
            ...
            <div style={S.formGrid}>
              ...
              <RelationshipPicker label="Children" entries={allPersons.filter(p => { if (p.id === personForm.editId || p.id === personForm.pSpouseId) return false; if (personForm.pChildIds.includes(p.id)) return true; if (personForm.editId && p.childIds.includes(personForm.editId)) return false; const cid = personForm.editId || '__new__'; if (getAncestorIds(cid, allPersons).has(p.id)) return false; if (getDescendantIds(p.id, allPersons).has(cid)) return false; return allPersons.filter(o => o.id !== personForm.editId && o.id !== personForm.pSpouseId && o.childIds.includes(p.id)).length < 2; })} selectedIds={personForm.pChildIds} onToggle={id => personForm.setPChildIds(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])} />
            ...
            <div style={{ ...S.btnRow, marginTop: 18 }}><button style={{ ...S.btn, ...S.btnPrimary }} onClick={personForm.save}>Save Person</button><button style={{ ...S.btn, ...S.btnSec }} onClick={() => { personForm.reset(); setError(''); setView('list'); }}>Cancel</button></div>
          </>
        )}
        ...
        {view === 'companyForm' && (
          <>
            ...
            <div style={{ ...S.btnRow, marginTop: 18 }}><button style={{ ...S.btn, ...S.btnPrimary }} onClick={companyForm.save}>Save Company</button><button style={{ ...S.btn, ...S.btnSec }} onClick={() => { companyForm.reset(); setError(''); setView('list'); }}>Cancel</button></div>
          </>
        )}
      ...
      {/* ── DIALOGS ── */}
      {deleteTarget && <DeleteDialog target={deleteTarget} allPersons={allPersons} entries={entries} onCancel={() => setDeleteTarget(null)} onConfirm={actions.confirmDelete} />}
      {showDeleteAll && <DeleteAllDialog entryCount={entries.length} householdCount={households.length} onCancel={() => setShowDeleteAll(false)} onConfirm={actions.handleDeleteAll} />}
      {showPrintDialog && <PrintDialog entries={entries} households={households} images={images} onClose={() => setShowPrintDialog(false)} onError={setError} />}
    </div>
  );
};
...
```
webapp/src/
├── App.tsx                          # Thin orchestration shell (~374 lines)
├── types.ts                         # Interfaces, type aliases, constants
├── styles.ts                        # Colors and style definitions
├── utils.ts                         # Utility functions and custom hooks
├── storage.ts                       # Storage operations, CSV helpers, image CRUD
...
│   ├── usePersonForm.ts             # Person form state, validation, save
...
echo "✅ Done! Extracted 39 files into $ROOT/"
