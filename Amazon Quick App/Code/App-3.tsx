...
type View = 'list' | 'personForm' | 'companyForm' | 'detail' | 'import';
type SortField = 'name' | 'type' | 'dateAdded';
...
function entryToCsvRow(e: DirectoryEntry): Record<string, string> {
...
function csvRowToEntry(row: Record<string, string>): DirectoryEntry | null {
  // If _json column exists and is valid, use it for lossless round-trip
  if (row._json) {
    try {
...
function DuplicateWarningBanner({ duplicates, onViewEntry }: {
  ...
  return (
    <div style={{
      background: '#fef7e0', border: '1px solid #f9c642', borderRadius: 8,
      ...
      {duplicates.map(d => (
        <div key={d.entry.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6, fontSize: 13 }}>
          ...
          <button
            style={{ ...S.btn, padding: '2px 10px', fontSize: 12, ...S.btnSec, marginLeft: 'auto' }}
            onClick={() => onViewEntry(d.entry.id)}
          >
            View
          </button>
        </div>
...
export const App = () => {
  ...
  // Duplicate detection
  const personDuplicates = React.useMemo(() =>
    findDuplicates(entries, editId, `${pFirst} ${pLast}`,
      getPrimary(pEmails)?.address || '', getPrimary(pPhones)?.number || '', 'person'),
    [entries, editId, pFirst, pLast, pEmails, pPhones]
  );
  const companyDuplicates = React.useMemo(() =>
    findDuplicates(entries, editId, cName,
      getPrimary(cEmails)?.address || '', getPrimary(cPhones)?.number || '', 'company'),
  ...
  // Search filtering + Sort/Filter pipeline
  ...
  const getEntryName = (e: DirectoryEntry) =>
  ...
  const filteredEntries = React.useMemo(() => {
    ...
    // Industry filter (applies to companies; if set, persons are excluded)
    ...
    list = [...list].sort((a, b) => {
      let cmp = 0;
      if (sortField === 'name') {
        cmp = getEntryName(a).toLowerCase().localeCompare(getEntryName(b).toLowerCase());
      } else if (sortField === 'type') {
        cmp = a.type.localeCompare(b.type) || getEntryName(a).toLowerCase().localeCompare(getEntryName(b).toLowerCase());
      } else {
        // dateAdded: sort by id (UUIDs are time-ordered in v4 only loosely, but this provides stable order)
...
## 10. CSV Export & Import Feature
- **Export**: "📥 Export CSV" button on the list view exports all directory entries to a \`directory-export.csv\` file
  - CSV includes flat columns (type, name, email, phone, address, relationships, notes) for human readability
  - Also includes a \`_json\` column with the full JSON of each entry for lossless round-trip import
  - Uses PapaParse library for CSV generation
- **Import**: "📤 Import CSV" button opens a file picker for \`.csv\` files
  - Supports both lossless round-trip (via \`_json\` column) and flat column parsing (for manually-edited CSVs)
  - Shows a preview table with entry count, type badges, names, and primary contact info
  - User reviews the preview and clicks "Import" to save all entries to storage
  - Parse errors are surfaced in the error banner (up to 5 shown)
  - New UUIDs are generated for flat-parsed entries; \`_json\`-based entries retain original IDs
- **Import View**: Dedicated view with table preview, confirm, and cancel buttons
`;
    try {
  ...
  const handleExportCsv = async () => {
    ...
    try {
      await downloadFile('directory-export.csv', new Blob([csv], { type: 'text/csv' }));
    } catch (e: any) {
  ...
  const handleImportConfirm = async () => {
    ...
    try {
      ...
      // Brief success feedback via a temporary message
        ...
        {view === 'list' && (
          <>
            <div style={S.btnRow}>
              ...
              <div style={{ flex: 1 }} />
              <button style={{ ...S.btn, ...S.btnSec }} onClick={handleExportCsv} title="Export all entries to CSV">📥 Export CSV</button>
              <button style={{ ...S.btn, ...S.btnSec }} onClick={() => fileInputRef.current?.click()} title="Import entries from CSV">📤 Import CSV</button>
        ...
        {view === 'import' && (
          <>
            ...
            <p style={{ fontSize: 13, color: colors.textSec, marginBottom: 16 }}>
              ...
              Review below, then confirm to save all entries.
            </p>
            {importPreview.length === 0 ? (
              <div style={S.emptyState}>No valid entries found in the CSV file.</div>
            ) : (
            ...
            <div style={{ ...S.btnRow, marginTop: 8 }}>
              <button style={{ ...S.btn, ...S.btnPrimary }} onClick={handleImportConfirm}
