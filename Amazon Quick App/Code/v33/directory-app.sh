...
import { S, colors } from './styles';
import { toE164, formatPhoneDisplay, ensureOnePrimary, getPrimary, getEntryName, formatAddr, calculateAge } from './utils';
      ...
      if (editId) {
        ...
        if (oldPerson && oldPerson.spouseId && oldPerson.spouseId !== pSpouseId) {
          ...
          if (oldSpouse && oldSpouse.type === 'person')
            await saveEntry({ ...oldSpouse, spouseId: '', weddingAnniversary: '' });
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (entry?.type === 'person') {
        // Unlink spouse and clear their wedding anniversary
        if (entry.spouseId) {
          const spouse = await loadEntry(entry.spouseId);
          if (spouse && spouse.type === 'person') await saveEntry({ ...spouse, spouseId: '', weddingAnniversary: '' });
        }
