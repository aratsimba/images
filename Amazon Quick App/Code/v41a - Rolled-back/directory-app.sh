...
interface HouseholdPickerProps {
  ...
  spouseId: string;
  childIds: string[];
  /** All household member IDs (manually managed) */
...
export function HouseholdPicker({
  ...
  const allMemberIds = extraMemberIds;
  ...
  return (
    <div style={S.fieldFull} ref={ref}>
      ...
      <span style={{ fontSize: 12, color: colors.textSec, display: 'block', marginBottom: 8 }}>
        Members share the same primary address. Add spouse, children, or others manually.
      </span>
      ...
      {allMemberIds.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', marginBottom: 8 }}>
          {allMemberIds.map(id => {
            ...
            return (
              <span key={id} style={{ ...S.chip, background: role ? '#e8f5e9' : colors.accent }}>
                {p.firstName} {p.lastName}
...
export const App = () => {
  ...
  const resetPersonForm = () => {
    ...
    setPSpouseId(''); setPChildIds([]); setPHouseholdId(uuidv4()); setPHouseholdExtraIds([]); setPNotes(''); setEditId(null);
  };
  ...
  const fillPersonForm = (p: Person) => {
    ...
    // Compute household members: all persons with same householdId (excluding self)
    const householdMembers = allPersons
      .filter(m => m.id !== p.id && m.householdId === p.householdId)
      .map(m => m.id);
    setPHouseholdExtraIds(householdMembers);
  ...
  const savePerson = async () => {
    ...
    // Collect all household member IDs (only explicitly added members)
    const householdMemberIds = [...new Set(pHouseholdExtraIds)];
    ...
    try {
      ...
      // Sync spouse's childIds (only if spouse is in household, sync householdId too)
      if (pSpouseId) {
        const spouse = await loadEntry(pSpouseId);
        if (spouse && spouse.type === 'person') {
          const spouseInHousehold = householdMemberIds.includes(pSpouseId);
          const newHouseholdId = spouseInHousehold ? pHouseholdId : spouse.householdId;
          const needsUpdate = spouse.spouseId !== id ||
            (spouseInHousehold && spouse.householdId !== newHouseholdId) ||
          ...
          <>
            ...
            <div style={S.formGrid}>
              ...
              <HouseholdPicker
                ...
                onAddMember={id => setPHouseholdExtraIds(prev => [...prev, id])}
