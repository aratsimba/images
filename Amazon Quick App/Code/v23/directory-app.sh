  ...
  return (
    <div style={S.fieldFull} ref={ref}>
      ...
      <span style={{ fontSize: 12, color: colors.textSec, display: 'block', marginBottom: 8 }}>
        Members share the same primary address. Spouse and children are automatically included but can be removed.
      </span>
          ...
          {allMemberIds.map(id => {
            ...
            return (
              <span key={id} style={{ ...S.chip, background: isAuto ? '#e8f5e9' : colors.accent }}>
                ...
                {isAuto ? (
                  <button style={S.chipRemove} title={`Remove ${p.firstName} from household`}
                    onClick={() => { if (id === spouseId) onRemoveSpouse(); else onRemoveChild(id); }}>×</button>
                ) : (
...
export const App = () => {
  ...
  const savePerson = async () => {
    ...
    // Collect all household member IDs (spouse + children + extras)
    ...
    try {
      ...
      // Sync spouse's childIds and householdId
      if (pSpouseId) {
        const spouse = await loadEntry(pSpouseId);
        if (spouse && spouse.type === 'person') {
          const needsUpdate = spouse.spouseId !== id ||
            spouse.householdId !== pHouseholdId ||
            JSON.stringify([...spouse.childIds].sort()) !== JSON.stringify([...pChildIds].sort());
          if (needsUpdate)
            await saveEntry({ ...spouse, spouseId: id, householdId: pHouseholdId, childIds: pChildIds });
        }
      }
          ...
          <>
            ...
            <div style={S.formGrid}>
              ...
              <HouseholdPicker
                ...
                onRemoveSpouse={() => setPSpouseId('')}
                onRemoveChild={id => setPChildIds(prev => prev.filter(x => x !== id))}
              />
