...
export const App = () => {
  ...
  const savePerson = async () => {
    ...
    if (invalidEmail) { setError('Please fix invalid email addresses before saving.'); return; }
    // Validate phones
    const invalidPhone = pPhones.find(p => !isValidPhone(p.number));
    if (invalidPhone) { setError('Please fix invalid phone numbers before saving.'); return; }
    ...
    if (incompleteAddr) { setError('Please complete all addresses — at least city and state are required.'); return; }
    setError('');
  ...
  const saveCompany = async () => {
    ...
    if (invalidCompanyEmail) { setError('Please fix invalid email addresses before saving.'); return; }
    // Validate phones
    const invalidCompanyPhone = cPhones.find(p => !isValidPhone(p.number));
    if (invalidCompanyPhone) { setError('Please fix invalid phone numbers before saving.'); return; }
    // Validate addresses: any address with content must have city and state
    const incompleteCompanyAddr = cAddrs.find(a => (a.street.trim() || a.city.trim() || a.state.trim() || a.zip.trim()) && (!a.city.trim() || !a.state.trim()));
    if (incompleteCompanyAddr) { setError('Please complete all addresses — at least city and state are required.'); return; }
    // Validate website
    if (cWebsite.trim()) {
      try {
        ...
        {view === 'companyForm' && (
          <>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <input style={{ ...S.input, ...(cWebsiteError ? { borderColor: colors.danger } : {}) }} type="url" placeholder="https://www.example.com" value={cWebsite}
            ...
            <MultiAddressFields addresses={cAddrs} onChange={setCAddrs} companyMode />
...
echo "✅ Done! Extracted 24 files into $ROOT/"
echo ""
echo "To get started:"
echo "  cd $ROOT"
echo "  npm install"
