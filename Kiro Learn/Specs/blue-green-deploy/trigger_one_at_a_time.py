#!/usr/bin/env python3
"""Trigger a OneAtATime blue-green deployment via CodeDeploy.

This script deploys v3.zip using CodeDeployDefault.OneAtATime, which
installs the revision on one instance at a time — the most conservative
EC2 deployment strategy, analogous to a linear rollout.

Usage:
    python3 trigger_one_at_a_time.py
"""
...
REVISION_KEY = "revisions/v3.zip"


def main():
    print("=" * 60)
    print("  Blue-Green Deployment — OneAtATime")
    print("=" * 60)
    ...
    print(f"\n→ Triggering OneAtATime deployment...")
    print(f"  Application:       {APP_NAME}")
    print(f"  Deployment Group:  {DEPLOYMENT_GROUP}")
    print(f"  Config:            {DEPLOYMENT_CONFIG}")
    print()
    print("  OneAtATime deploys to one instance at a time,")
    print("  providing the most conservative rollout — similar to linear.")
    print()
    ...
    deployment_id = create_deployment(
        ...
        description="OneAtATime blue-green deployment of v3",
    )
    ...
    print(f"  Strategy: OneAtATime — instances deployed one by one")
    print("=" * 60)
