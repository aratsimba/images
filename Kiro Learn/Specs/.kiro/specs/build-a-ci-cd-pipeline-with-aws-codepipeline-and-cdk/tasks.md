...
- [x] 6. Multi-Environment Deployment and Approval Gate
  ...
  - [x] 6.3 Property Test - Pipeline Stage Ordering
...
- [x] 7. Validation Steps - Linting and Security Scanning
  - [x] 7.1 Create Lint and Security Scan Steps (Component 5: BuildSpecConfigs - createLintStep, createSecurityScanStep)
    ...
    - Implement `createSecurityScanStep(source)` returning a `ShellStep` that installs cfn-nag and runs `cfn_nag_scan` against only `*.template.json` files in `cdk.out/` (skipping non-CloudFormation files like `manifest.json` and `tree.json`)
...
- [x] 8. Checkpoint - Full Pipeline Deployment and Self-Mutation
...
- [x] 9. End-to-End Pipeline Verification
  - [x] 9.1 Trigger Full Pipeline Execution
  ...
  - [x] 9.2 Approve and Verify Production Deployment
...
- [x] 10. Cleanup - Resource Teardown
  - [x] 10.1 Delete Pipeline and Application Stacks
    ...
    - _Requirements: (all)_
  - [x] 10.2 Delete CodeCommit Repository and Bootstrap Resources
